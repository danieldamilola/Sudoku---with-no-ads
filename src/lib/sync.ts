// ─── Supabase Sync Utility (Mobile) ───────────────────────────────────────────────
import { supabase, type Database } from './supabase';
import { GameSettings, GameStats, GameState } from '../types';
...
function mapRemoteStats(remote: UserStats, base: GameStats): GameStats {
  ...
  const { _wins, ...bestTimes } = rawTimes as Record<string, number> & { _wins?: Record<string, number> };
  return {
    ...base,
...
export async function pushStats(userId: string, stats: GameStats): Promise<void> {
  ...
  const { error } = await supabase
    ...
    .upsert({
      ...
      best_times: { ...stats.bestTimes, _wins: stats.winsByDifficulty },
    });
