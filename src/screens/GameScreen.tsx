...
import { useStore, MAX_SECOND_CHANCES } from "../store/useStore";
...
const Btn = ({
  ...
  <TouchableOpacity
    ...
    style={[
      S.actBtn,
      active && { backgroundColor: themeColors.accentDim },
...
export const GameScreen: React.FC = () => {
  ...
  const mistakeText =
    mistakeLimit > 0 ? `${mistakes}/${mistakeLimit}` : `${mistakes}`;
  ...
  // ── Auto-pause + cloud save when app goes to background ──────────────────────
  useEffect(() => {
  ...
  const related = useMemo(() => {
    ...
    return new Set(
      SudokuValidator.getRelatedCellsIndices(
        selectedCell.row,
        selectedCell.col,
      ),
    );
  ...
  const sameNum = useMemo(() => {
    if (!selectedCell?.value || !settings.highlightDuplicates)
      return new Set<number>();
    return new Set(
      SudokuValidator.getSameNumberCells(cells, selectedCell.value),
    );
  ...
  const { completedNumbers, remainingCounts } = useMemo(() => {
    ...
    for (let n = 1; n <= 9; n++) {
      rem[n] = 9 - (counts[n] ?? 0);
      if (rem[n] <= 0) done.add(n);
    }
    return { completedNumbers: done, remainingCounts: rem };
  }, [cells]);
  const anims = useRef<Record<number, Animated.Value>>({});
  const shakeAnims = useRef<Record<number, Animated.Value>>({});

  const numpadAnims = useRef<Record<number, Animated.Value>>({});
  const animCell = (idx: number) => {
    ...
    Animated.sequence([
      ...
      Animated.spring(anims.current[idx], {
        toValue: 1,
        friction: 4,
        tension: 180,
        useNativeDriver: true,
  ...
  const shakeCell = (idx: number) => {
    if (!shakeAnims.current[idx])
      shakeAnims.current[idx] = new Animated.Value(0);
  ...
  /* Animate numpad key when a digit becomes fully completed */
  useEffect(() => {
    const prev = prevCompletedRef.current;
    completedNumbers.forEach((n) => {
      if (!prev.has(n)) {
        if (!numpadAnims.current[n]) numpadAnims.current[n] = new Animated.Value(1);
        const anim = numpadAnims.current[n];
        anim.setValue(1);
        Animated.sequence([
    ...
    prevCompletedRef.current = new Set(completedNumbers);
  }, [completedNumbers]);
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <View style={[S.header, { height: H_HEADER }]}>
        ...
        >
          <Text style={[S.diffBadgeTxt, { color: colors.textPrimary }]}>
            {diffLabel}
          </Text>
        </View>
      ...
      <View style={{ alignItems: "center", marginBottom: 8 }}>
        <Board
          ...
          onCell={(idx) => {
            useStore.getState().selectCell(idx);
            animCell(idx);
          }}
        />
      </View>
      ...
      <View style={[S.actRow, { height: H_ACTIONS }]}>
        <Btn
          LIcon={Eraser}
          label="Erase"
          onPress={() => useStore.getState().eraseCell()}
          disabled={isPaused}
          themeColors={colors}
        />
        <Btn
          LIcon={PencilLine}
          label="Notes"
          onPress={() => useStore.getState().toggleNotesMode()}
          disabled={isPaused}
          active={notesMode}
          themeColors={colors}
        />
        <Btn
          ...
          onPress={() => useStore.getState().useHint()}
          disabled={isPaused || remainingHints === 0}
          themeColors={colors}
        />
        <Btn
          LIcon={RotateCcw}
          label="Undo"
          onPress={() => useStore.getState().undoMove()}
          disabled={isPaused || noUndo}
          themeColors={colors}
        />
      </View>
      ...
      <View style={[S.numPad, { paddingHorizontal: PAD }]}>
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => {
          ...
          const rem = remainingCounts[n] ?? 0;
          return (
            <Animated.View
              key={n}
              style={[
                { width: numBtnW, height: numBtnH },
                done && numpadAnims.current[n]
                  ? { transform: [{ scale: numpadAnims.current[n] }] }
                  : undefined,
              ]}
            >
              {!done && (
                ...
                >
                  <Text style={[S.numTxt, { color: colors.textPrimary }]}>
                    {n}
                  </Text>
                  {rem < 9 && (
                    <Text
      ...
      >
        <View style={S.overlay}>
          <View style={[S.pauseCard, { backgroundColor: colors.bgSurface }]}>
            ...
            <Text style={[S.pauseTitle, { color: colors.textPrimary }]}>
              Game Paused
            </Text>
            <Text style={[S.pauseTime, { color: colors.textSecondary }]}>
              {formatTime(elapsed)}
            </Text>

            {/* Difficulty pill */}
            <View
              style={[S.pauseDiffPill, { borderColor: colors.outlineVariant }]}
            >
            ...
            <View style={S.pauseBtnStack}>
              ...
              >
                ...
                <Text style={[S.pauseBtnTxt, { color: colors.onPrimary }]}>
                  Resume
                </Text>
              </TouchableOpacity>
              ...
              >
                ...
                <Text style={[S.pauseBtnTxt, { color: colors.textPrimary }]}>
                  Go to Home
                </Text>
              </TouchableOpacity>
            </View>
      ...
      >
        <View style={S.sheetOverlay}>
          ...
          >
            <View
              style={[S.dragHandle, { backgroundColor: colors.outlineVariant }]}
            />
            ...
            <View
              style={[
                S.successIcon,
                { backgroundColor: colors.error, marginBottom: 16 },
              ]}
            >
            ...
            <View style={[S.diffPill, { borderColor: colors.outlineVariant }]}>
              <Text style={[S.diffPillTxt, { color: colors.textSecondary }]}>
            ...
            <View style={S.resultRow}>
              {[
                ...
                <View
                  key={label}
                  style={[S.resultCard, { borderColor: colors.outlineVariant }]}
                >
                  <Text style={[S.resultLbl, { color: colors.textSecondary }]}>
                    {label}
                  </Text>
                  <Text style={[S.resultVal, { color: colors.textPrimary }]}>
                    {value}
                  </Text>
                </View>
              ))}
            ...
            {secondChancesUsed < MAX_SECOND_CHANCES && (
              <View style={S.chancePillRow}>
                ...
                <Text style={[S.chanceTxt, { color: colors.textSecondary }]}>
                  {MAX_SECOND_CHANCES - secondChancesUsed} chance
                  {MAX_SECOND_CHANCES - secondChancesUsed !== 1 ? "s" : ""} left
                </Text>
              </View>
            )}
            ...
            {secondChancesUsed < MAX_SECOND_CHANCES ? (
              ...
              >
                ...
                >
                  <RotateCcw size={18} color={colors.onSecondary} strokeWidth={2} />
                  <Text style={[S.primaryBtnTxt, { color: colors.onSecondary }]}>
                    Second Chance
                  </Text>
                </View>
              </TouchableOpacity>
              ...
              >
                <Text style={[S.noChanceTxt, { color: colors.textSecondary }]}>
            ...
            <TouchableOpacity
              style={[
                S.primaryBtn,
                {
                  backgroundColor:
                    secondChancesUsed < MAX_SECOND_CHANCES
                      ? colors.bgSurfaceContainerLowest
                      : colors.primary,
...
const S = StyleSheet.create({
  ...
  statValMono: { fontSize: 17, fontWeight: "700", fontFamily: "DM Mono" },
  ...
  primaryBtnTxt: { fontSize: 17, fontWeight: "700" },
